...
exposure_dat <- NULL
#try(exposure_dat <- read_excel("./data/pQTL-5studies+Paola.xlsx", 3))

try(exposure_dat <- read.table("./instruments/pQTL-instruments-five-studies.txt",header=T,stringsAsFactors=FALSE, colClasses = c("character")))

#try(exposure_dat <- read.table("./instruments/CHI3L1.ins.txt",header=T,stringsAsFactors=FALSE, colClasses = c("character")))

exposure_dat$pval.exposure <- as.numeric(exposure_dat$pval)
...
for (i in 4:length(ids)){
  ...
  exposure <- NULL
  outcome <- NULL 
  exposure <- "pQTL-covid19-2021-06"
  #exposure <- "pQTL-CHI3L1"
  outcome <- as.character(as.vector(strsplit(ids[i], "/")[[1]][6]))
  result_file0 <- paste0("./results/org-results/pQTL/",exposure,".",outcome,".harmonise.txt")
  result_file <- paste0("./results/org-results/pQTL/",exposure,".",outcome,".mr.txt")
  result_file2 <- paste0("./results/org-results/pQTL/",exposure,".",outcome,".mr_hetero.txt")
  result_file3 <- paste0("./results/org-results/pQTL/",exposure,".",outcome,".mr_pleio.txt")
  result_file4 <- paste0("./results/org-results/pQTL/",exposure,".",outcome,".mr_single.txt")
