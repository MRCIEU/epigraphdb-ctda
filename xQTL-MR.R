...
try(exposure_dat <- read.table("./instruments/xqtl-covid19-related-instruments.txt",header=T,stringsAsFactors=FALSE, colClasses = c("character")))

exposure_dat$pval.exposure <- as.numeric(exposure_dat$pval)

exposure_dat <-format_data(exposure_dat, type = "exposure", header = TRUE,
                             phenotype_col = "exposure", snp_col = "SNP", beta_col = "beta.exposure",
...
for (i in 4:length(ids)){
  ...
  exposure <- NULL
  outcome <- NULL 
  exposure <- "xQTL-covid19-2021-06"
  outcome <- as.character(as.vector(strsplit(ids[i], "/")[[1]][6]))
  result_file0 <- paste0("./results/org-results/xQTL/",exposure,".",outcome,".harmonise.txt")
  result_file <- paste0("./results/org-results/xQTL/",exposure,".",outcome,".mr.txt")
  result_file2 <- paste0("./results/org-results/xQTL/",exposure,".",outcome,".mr_hetero.txt")
  result_file3 <- paste0("./results/org-results/xQTL/",exposure,".",outcome,".mr_pleio.txt")
  result_file4 <- paste0("./results/org-results/xQTL/",exposure,".",outcome,".mr_single.txt")
